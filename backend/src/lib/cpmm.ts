...
export interface CpmmPool {
  YES: number  // In cents (BIGINT)
  NO: number   // In cents (BIGINT)
}
...
export interface BetResult {
  ...
  newPool: CpmmPool
  fees: number
  amount: number // Amount after fees
}
 ...
 */
export function getCpmmProbability(pool: CpmmPool, p: number): number {
  const { YES, NO } = pool
...
export function getCpmmFees(
  ...
  betAmount: number,
  outcome: 'YES' | 'NO'
): { remainingBet: number; totalFees: number } {
  if (betAmount === 0) {
 ...
 */
export function validatePoolInvariant(
  oldPool: CpmmPool,
  newPool: CpmmPool,
  p: number
): boolean {
 ...
 */
export function getNoProbability(pool: CpmmPool, p: number): number {
